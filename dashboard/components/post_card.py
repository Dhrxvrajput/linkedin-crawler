    ...
    with st.container(border=True):
        ...
        <div style="display:flex; justify-content:space-between; align-items:start; margin-bottom: 8px;">
            <div style="display:flex; align-items:center; gap:10px;">
                ...
                ">{initials}</div>
                <div>
                    <div style="font-weight:600; font-size:1.05rem; color:#E8E8ED; line-height:1.2;">
                        {name}
                    </div>
                    <div style="font-size:0.86rem; color:#9D9DB7; line-height:1.3; margin-top:2px;">
        ...
        calendar_svg = '<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-calendar" style="vertical-align: -2px; margin-right: 4px; color: #9D9DB7;"><path d="M8 2v4"/><path d="M16 2v4"/><rect width="18" height="18" x="3" y="4" rx="2"/><path d="M3 10h18"/></svg>'
        calendar_html = clean_html(f'<div style="font-size:0.86rem; color:#9D9DB7; display:flex; align-items:center; gap:2px; margin-bottom: 8px;">{calendar_svg}{posted}</div>')
        ...
        if post.get("engagement_comment") or post.get("summary"):
            ...
            if post.get("engagement_comment"):
                comment_block = f"""
                <div style="font-size:0.96rem; color:#E8E8ED; margin-top:8px; line-height:1.45;">
                    <span style="color:#63B3ED; font-weight:600;">Draft Comment:</span> {post['engagement_comment']}
                </div>
                <div style="font-size:0.86rem; color:#9D9DB7; margin-top:6px; display:inline-flex; align-items:center; gap:4px;">
            ...
            if post.get("summary"):
                summary_block = f"""
                <details style="margin-top: 10px; font-size:0.9rem; color:#9D9DB7; cursor:pointer;">
            ...
            <div style="
                ...
                margin: 8px 0;
            ">
                <div style="font-weight:600; font-size:.92rem; color:#5da9ff; display:flex; align-items:center; gap:6px; margin-bottom: 2px;">
        ...
        # Subtle separator
        sep_html = clean_html("<hr style='border:none; height:1px; background:linear-gradient(90deg, rgba(10, 102, 194, 0.15), transparent); margin: 10px 0 !important;' />")
        st.markdown(sep_html, unsafe_allow_html=True)

        # Content Header
        content_header_html = clean_html(f"""
        <div style="font-weight:600; font-size:0.95rem; color:#E8E8ED; margin: 4px 0 6px; display:flex; align-items:center; gap:6px;">
        ...
        """)
        st.markdown(content_header_html, unsafe_allow_html=True)
        ...
        if len(content) > limit:
            ...
            ">
                ...
                <details style="margin-top: 6px; font-size: 1.02rem;">
            ...
            """)
            st.markdown(content_box_html, unsafe_allow_html=True)
        else:
            content_box_html = clean_html(f"""
            <div style="
                background: rgba(255,255,255,0.015);
        ...
        profile_button_html = ""
        if profile_url:
            profile_button_html = clean_html(f"""
        ...
        metrics_html = clean_html(f"""
        <div style="display:flex; justify-content:space-between; align-items:center; margin-top:12px; gap:10px;">
            <div style="display:flex; gap:10px; align-items:center;">
                <div style="display:flex; align-items:center; gap:5px; background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.05); padding:5px 10px; border-radius:6px; font-size:0.86rem;">
