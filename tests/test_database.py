...
class TestDatabase:
    ...
    def test_get_posts_excludes_irrelevant(self):
        with get_db() as db:
            create_post(db, PostSchema(
                id="post-relevant",
                author=PostAuthor(name="Alice", title="Eng"),
                content="Relevant post",
                domain="technology"
            ))
            create_post(db, PostSchema(
                id="post-irrelevant",
                author=PostAuthor(name="Bob", title="Dentist"),
                content="Irrelevant family post",
                domain="irrelevant"
            ))
            
            # Default get_posts should exclude post-irrelevant
            posts = get_posts(db)
            assert len(posts) == 1
            assert posts[0].id == "post-relevant"

            # Passing exclude_irrelevant=False should return both
            all_posts = get_posts(db, exclude_irrelevant=False)
